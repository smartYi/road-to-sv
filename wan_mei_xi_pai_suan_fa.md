...
再比如，2是9的原根，因为   ，为了让   除以9的余数恒等于1，可知最小的正整数d=6，而ϕ(m)=6，满足原根的定义。

定理1 同余定理：两个整数a，b，若它们除以正整数m所得的余数相等，则称a，b对于模m同余，记作   ，读做a与b关于模m同余。

定理2 当p为奇素数且a是   的原根时⇒ a也是   的原根
...
取头为1，（/27)*＝｛1,2,4,8,16,5,10,20,13,26,25,23,19,11,22,17,7,14｝，也就是说，与27互素且小于27的正整数集合为{1,2,4,8,16,5,10,20,13,26,25,23,19,11,22,17,7,14}，因此ϕ(m) = ϕ(27)=18, 从而满足   的最小d = 18，故得出2为27的原根；
